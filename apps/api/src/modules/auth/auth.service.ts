...
export async function register(input: {
  ...
  try {
    ...
    // The SELECT-then-INSERT check above has a narrow race: two
    // concurrent registrations with the same email can both pass the
    // SELECT before either INSERT commits. The unique index on
    // lower(email) (001_foundation.sql) still stops a duplicate at the
    // database level — this just turns that into the same 409 a
    // non-concurrent duplicate gets, instead of a raw constraint error
